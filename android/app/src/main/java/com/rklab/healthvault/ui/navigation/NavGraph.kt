...
private object Routes {
    ...
    const val LOCKER_ADD = "locker_add?type={type}&scan={scan}&folder={folder}&person={person}"
    ...
    fun lockerAdd(type: String? = null, scan: Boolean = false, folder: String? = null, person: String? = null) =
        "locker_add?type=${type ?: ""}&scan=${if (scan) "1" else "0"}&folder=${folder ?: ""}&person=${person ?: ""}"
        ...
        NavHost(
            ...
            composable(Routes.LOCKER) {
                LockerListScreen(
                    ...
                    onAdd = { type, folder, person -> navController.navigate(Routes.lockerAdd(type, folder = folder, person = person)) },
                    onScan = { type, folder, person -> navController.navigate(Routes.lockerAdd(type, scan = true, folder = folder, person = person)) },
                    onOpenModules = { navController.navigate(Routes.MODULES) }
                )
            }
            ...
            composable(
                ...
                val person = entry.arguments?.getString("person").orEmpty().ifBlank { null }
                    ?.takeIf { it != "none" }
                LockerAddScreen(
                    repository = repository,
                    defaultType = type,
